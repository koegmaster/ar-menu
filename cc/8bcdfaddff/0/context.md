# Session Context

## User Prompts

### Prompt 1

The inital app looks quiet ok, but to further improve the setup I would need to have the admin page behind a authenticatian and authorization layer, as currently everyone could see and use the data we upload. Another thing we need to take care about is that the 3d models created are way to big. we upload pictures of standard dishes on plates, and in AR they are as big as a table, so the scaling needs to be done properly.

### Prompt 2

yes please implment the plan now

### Prompt 3

the admin login page does not work at all https://ar-menu-pink-five.vercel.app/admin/login

### Prompt 4

Ok login worked, now I also want to have a progress bar for generating the 3d models directly in the admin panel, so that I do not need to manually check the status url all the time

### Prompt 5

Ok now there is an issue, the model generation goes to 99% gets stuck and then when I refresh it goes to 0% and stays there, so there is likely something wrong

